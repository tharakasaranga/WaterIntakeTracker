...
router.post("/notifications/preferences", async (req, res) => {
  try {
    ...
    console.log("[Notifications API] Saving preferences for user:", clerkId);
    ...
    let preference = await NotificationPreference.findOne({ clerkId });

    if (!preference) {
      preference = new NotificationPreference({
        clerkId,
        reminderFrequency: reminderFrequency || 120,
        reminderStartTime: reminderStartTime || "08:00",
        reminderEndTime: reminderEndTime || "22:00",
        enabled: enabled !== undefined ? enabled : true,
      });
    } else {
      if (reminderFrequency !== undefined)
        preference.reminderFrequency = reminderFrequency;
      if (reminderStartTime !== undefined)
        preference.reminderStartTime = reminderStartTime;
      if (reminderEndTime !== undefined)
        preference.reminderEndTime = reminderEndTime;
      if (enabled !== undefined) preference.enabled = enabled;
    }

    await preference.save();

    console.log("[Notifications API] Preferences saved successfully");
    ...
    console.error("[Notifications API] Error saving preferences:", err);
    res.status(500).json({ error: err.message });
  }
});

// GET: Get notification preferences
router.get("/notifications/preferences/:clerkId", async (req, res) => {
  try {
    const { clerkId } = req.params;

    console.log("[Notifications API] Fetching preferences for user:", clerkId);
    ...
    console.log("[Notifications API] Preferences retrieved:", preference);

    res.json(preference);
  } catch (err) {
    console.error("[Notifications API] Error fetching preferences:", err);
    res.status(500).json({ error: err.message });
  }
});

// POST: Quick add water from notification
router.post("/notifications/quick-add", async (req, res) => {
  try {
    const { clerkId, amount } = req.body;

    console.log("[Notifications API] Quick add water:", { clerkId, amount });
    ...
    const waterLog = new WaterLog({
      ...
      amount,
      date: new Date(),
      notes: "Added from notification quick action",
    });
    ...
    const notificationLog = new NotificationLog({
      ...
      action_details: { amount },
    });

    await notificationLog.save();

    console.log("[Notifications API] Water added successfully");

    res.json({
      success: true,
      waterLog,
      message: `${amount}ml added successfully`,
    });
  } catch (err) {
    console.error("[Notifications API] Error adding water:", err);
    res.status(500).json({ error: err.message });
  }
});

// GET: Get today's water intake data
router.get("/water-logs/today", async (req, res) => {
  try {
    const { clerkId } = req.query;

    console.log(
      "[Notifications API] Fetching today's water intake for:",
      clerkId,
    );
    ...
    // Get user info to find daily goal
    const user = await User.findOne({ clerkId });
    const dailyGoal = user?.dailyGoal || 2500;
    ...
    console.log("[Notifications API] Today's intake:", {
      totalIntake,
      dailyGoal,
    ...
    console.error("[Notifications API] Error fetching today's intake:", err);
    res.status(500).json({ error: err.message });
  }
});

// POST: Log notification event
router.post("/notifications/log", async (req, res) => {
  try {
    const { clerkId, type, title, body, user_action, action_details } =
      req.body;

    console.log("[Notifications API] Logging notification:", { clerkId, type });
    ...
    const log = new NotificationLog({
      ...
      title,
      body,
      user_action: user_action || "none",
      action_details,
    });

    await log.save();

    // Update last reminder sent time if it's a reminder
    if (type === "reminder") {
    ...
    console.log("[Notifications API] Notification logged successfully");

    res.json({ success: true, log });
  } catch (err) {
    console.error("[Notifications API] Error logging notification:", err);
    res.status(500).json({ error: err.message });
  }
});
...
router.post("/notifications/mark-read", async (req, res) => {
  try {
    const { clerkId, notificationId, action } = req.body;
    ...
    if (!clerkId || !notificationId) {
      return res.status(400).json({
        error: "clerkId and notificationId are required",
      });
    }

    const log = await NotificationLog.findByIdAndUpdate(notificationId, {
      user_action: action || "tapped",
    });

    console.log("[Notifications API] Notification marked");

    res.json({ success: true, log });
  } catch (err) {
    console.error("[Notifications API] Error marking notification:", err);
    res.status(500).json({ error: err.message });
  }
});

// GET: Get notification history
router.get("/notifications/history/:clerkId", async (req, res) => {
  try {
    const { clerkId } = req.params;

    console.log(
      "[Notifications API] Fetching notification history for:",
      clerkId,
    );
    ...
    const history = await NotificationLog.find({ clerkId })
      .sort({ sent_at: -1 })
      .limit(50);
